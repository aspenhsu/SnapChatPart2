    ...
    */
    func updateData() {
        ...
        getPosts(user: currentUser, completion: { (postsArray) in
            clearThreads()
            if (postsArray != nil){
                for hap in postsArray!{
                addPostToThread(post: hap)
                   let postIm = hap.postImagePath
                    getDataFromPath(path: postIm, completion: { (data) in
                        if (data != nil){
                        let newIm = UIImage(data: data!)
                            self.loadedImagesById[hap.postId] =  newIm
                        }
                    })
                }
