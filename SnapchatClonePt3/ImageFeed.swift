...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    dbRef.child(firPostsNode).childByAutoId().setValue(newDict)
...
func getPosts(user: CurrentUser, completion: @escaping ([Post]?) -> Void) {
    ...
    let readPost = user.getReadPostIDs(completion: {
        ...
        dbRef.child(firPostsNode).observeSingleEvent(of: .value, with: {
            (snapshot) in
        if (snapshot.exists()) {
            if let myDict2 = snapshot.value as? [String:AnyObject] {
                for key in myDict2.keys {
                    var boolbool: Bool
                    let id = key
                    let path:String = myDict2[key]!["imagePath"] as! String
                    let thread:String = myDict2[key]!["thread"] as! String
                    let date:String = myDict2[key]!["date"] as! String
                    let username = myDict2[key]!["username"] as! String
                    
                    if readArray.contains(key){
                        boolbool = true } else {
                    boolbool = false}
                    
                    let posty = Post.init(id: id, username: username, postImagePath: path, thread: thread, dateString: date, read: boolbool)
                    postArray.append(posty)
